...
# 1、物理总线
IIC 的物理总线有两条：
<br>（1）时钟线 SCL：由主机提供时钟信号
<br>（2）数据线 SDA：在时钟信号中交换数据或发送指令
<br>主机的 SCL、SDA 均为开漏模式。建议在这两根数据线上分别加入上拉电阻，稳定两个引脚被释放时的高电平，同时以合适的阻值限制流入芯片的电流大小。（3.3V-5V 供电的一般使用场景中，推荐使用 4.7kΩ）
<br>总线通过并联的形式挂载多个从机。通常情况下，要求主机在通信时发送指定的从机地址，使其进入通信状态，其他因地址不符而未进入通信状态的从机则不会响应。
<br>对于 SCL，在多数应用中，从机不主动控制 SCL，但 IIC 规范允许从机将 SCL 拉低让主机等待。本文暂不讨论，则从机始终为高阻态，只有主机通过发送低电平脉冲向各个从机提供同步信号。
<br>对于 SDA，发送的一方可选择拉低电平或以高阻态释放该线为高电平，接收的一方则保持高阻态以读取电平状态。因此 SCL 上只有主机发出的单向信号，SDA 上为双向信号。
...
# 2、通信时序
从电平逻辑上概括的话，接收方器件在 SCL 为高电平时对 SDA 采样，然后依情况解析成数据或指令：
...
- SCL 低电平期间，发送方器件可按需更改 SDA 的电平，为实现 SCL 高电平期间的电平动作或状态做准备。
...
u8 IIC_ReceiveACK(void) {
    ...
    while(TimeOut--) {
        ...
        IIC_Delay5us();    // <-- 此处可以修改延时函数，但延时过长会严重影响通信速度。以本代码为例，假设此处延时 5us，那么代码实际上会每隔 5us 检查一次 SDA，若有应答则提前跳出轮询的循环，函数最后会返回应答（返回 0）；否则轮询循环会在 200 次轮询后结束，超时时间一共为 200 * 5us = 1ms，函数最后会返回不应答（返回 1）
...
## 3.3 通信速度适配优化
受器件接口和布线等原因附带的 RC 特性影响，总线上的电平被主机或从机翻转后，需要一定的时间才能稳定，从机的逻辑处理单元对于信号的处理速度也有上限，两者是限制 IIC 通信速度的主要因素。（除非主机自身的电平操作速度已经足够慢。若主机速度足够慢，本节所述优化可忽略）
<br>为了使通信在足够快速的同时保证可靠性，在布线尽量短、保护总线尽量不受电磁干扰、依据总线挂载情况选择合适上拉电阻的同时，还需要从单片机程序中主动限制单片机的引脚操作速度。
<br>以绝大多数器件支持的标准模式 100kHz 为例，可以在每次编辑引脚电平的代码后延时 5us。那么代码形式参考中可以封装引脚函数：
```C++
...
void IIC_EditSCL(u8 Dat) {
    ...
    IIC_Delay5us();    // <-- 先更改引脚电平，再延时，这个顺序能利用延时等待电平稳定，此后紧挨的采样电平代码才能采样到更确定的电平，同下
}
...
u8 IIC_ReceiveACK(void);        // 接收应答
void IIC_SendByte(u8 Byte);     // 发送字节
u8 IIC_ReceiveByte(void);       // 接收字节

#endif
```
```C++
// 控制电平翻转速度的延时函数
static void IIC_Delay5us(void) {
    // 5us 延时代码
}
...
// 电平控制封装函数
static void IIC_EditSCL(u8 Dat) {
    if(Dat) {SCL = 1;}
    else {SCL = 0;}
    IIC_Delay5us();
}
static void IIC_EditSDA(u8 Dat) {
    if(Dat) {SDA = 1;}
    else {SDA = 0;}
    IIC_Delay5us();
}
